...
def add_path(df, f_p, v_p):
    ...
    for i in df_copy.index:
        df_copy.loc[i, 'path'] = f_p + df_copy.loc[i, 'video_id'] + '/' + df_copy.loc[i, 'video_id'] + '.npy'
        # os.mkdir(f_p+df.loc[i,'video_id'])
        df_copy.loc[i, 'video_path'] = v_p + df_copy.loc[i, 'video_id'] + '/' + df_copy.loc[i, 'video_id'] + '.mp4'
    return df_copy


# csv에 path 추가하기
df = pd.read_csv("TK/data/HowTo100M_v1_199.csv")
f_p = 'TK/Howto100M/howto100m_video/parsed_video'  # feature path
v_p = 'TK/Howto100M/howto100m_video/videos'  # video path
df_f = add_path(df,f_p, v_p)
df_f.to_csv("TK/data/HowTo100M_v1_199.csv", index=False)
